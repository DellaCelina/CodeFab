...
TEST(CheckerTest, DuplicateDeclarationInSameScopeReportsError) {
    ...
    auto declA1 = std::make_unique<VarDeclStatement>(testTokens(TokenType::VAR, "var", 2), "a", lit10.get());

    auto lit12 = std::make_unique<NumberExpression>(testTokens(TokenType::NUMBER, "12", 3), 12.0);
    auto declA2 = std::make_unique<VarDeclStatement>(testTokens(TokenType::VAR, "var", 3), "a", lit12.get());
...
TEST(CheckerTest, SelfReferenceInInitializerReportsError) {
    ...
    auto declA = std::make_unique<VarDeclStatement>(testTokens(TokenType::VAR, "var", 2), "a", add.get());
